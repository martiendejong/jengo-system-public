...
.DESCRIPTION
    ...
    4. Updates pool.md
    5. Logs activity
    6. Optionally allocates paired framework worktree

.PARAMETER Repo
    Repository to allocate (must be configured in $BaseRepos)
...
.PARAMETER Paired
    Also allocate paired framework worktree (for app work that depends on framework)

.PARAMETER Description
    Short description of what you're working on

.EXAMPLE
    .\worktree-allocate.ps1 -Repo my-app -Branch feature/new-thing
    .\worktree-allocate.ps1 -Repo my-app -Branch feature/x -Paired -Description "Adding PDF export"
    .\worktree-allocate.ps1 -Repo my-framework -Branch fix/bug -Seat agent-003
#>
...
function Get-FreeSeat {
    ...
    $newRow = "| $newSeat | $($newSeat.Replace('-','')) | $env:PROJECTS_ROOT | $WorkerAgentsPath\$newSeat | FREE | - | - | $(Get-Date -Format 'yyyy-MM-ddTHH:mm:ssZ') | Auto-provisioned |"
...
if ($Paired) { Write-Host "Mode: PAIRED (app + framework)" }
Write-Host ""
...
try {
    ...
    if ($Paired -and $PairedRepo) {
        Write-Step "Checking paired repo base state..." "INFO"
        Test-BaseRepoState -RepoPath $BaseRepos[$PairedRepo]
    }
    ...
    if ($Paired -and $PairedRepo) {
        Write-Step "Creating paired worktree..." "INFO"
        $pairedPath = New-Worktree -SeatName $Seat -RepoName $PairedRepo -BranchName $Branch
        Write-Step "Created: $pairedPath" "OK"
    }
    ...
    if ($Paired -and $PairedRepo) {
        Log-Activity -SeatName $Seat -RepoName $PairedRepo -BranchName $Branch
    }
    ...
    if ($Paired -and $PairedRepo) {
        Write-Host "Paired:  $WorkerAgentsPath\$Seat\$PairedRepo" -ForegroundColor White
    }
    Write-Host ""
